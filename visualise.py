...
### Create the logger
logger = logging.getLogger('visualise_log')
logger.setLevel(logging.DEBUG)
timestamp = datetime.now().strftime("%d_%b_%Y_%H:%M:%S")

def countEventIDs(logfile_path):
...
def getEventIDLists(logfile_path):
    eventIDList = list()
...
def drawGraphs(eventIDList, eventIDCountList, logfileName):
...
def main():
    ...
    logfileName = "visualise_log_" + timestamp
    logfilePath = os.path.join(LOGFILE_PATH, logfileName)
    ...
    analyseLogfileName = input('Please specify the log file name to start analysing and visualisation process:\n')
    analyseLogfilePath = os.path.join(LOGFILE_PATH, analyseLogfileName)
    
    # Create the logfile and write the logs in it.
    eventIDDict = countEventIDs(analyseLogfilePath)
    
    # Log the dictionary to logfile
    for eventID in eventIDDict:
        logger.info("Event ID: {event_id}".format(event_id=eventID))
        logger.info("Event Count: {count}".format(count=eventIDDict.get(eventID).get("Count")))
        logger.info("")
    
    eventIDList, eventIDCountList = getEventIDLists(logfilePath)
    drawGraphs(eventIDList, eventIDCountList, logfileName)
